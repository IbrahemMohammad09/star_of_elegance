...
const Services = () => {
  ...
  useEffect(()=>{
    const fetchData = async ()=>{
      try{
        const response = await axios.get("https://starofelegance.pythonanywhere.com/api/services/");
        console.log("📌 البيانات المسترجعة:", response.data); // تحقق من نوع البيانات
    ...
    <div className="pt-40 flex justfiy-center flex-col">
      ...
      <div>
        ...
        {/* {services.map((service) => (
          <li key={service.id}>
            <h3>{service.name}</h3>
            <img src={service.picture} alt={service.name} width="200" />
